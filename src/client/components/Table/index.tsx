...
import { translateColumns } from "./utils";
...
const Table = <T, _>({
  ...
  const { getRowModel, getHeaderGroups } = useReactTable({
    columns: translateColumns({ columns: filteredColumns, entity, schema, columnSizing }),
    data,
          ...
          {getHeaderGroups().map((headerGroup) => (
            <tr key={headerGroup.id}>
              ...
              <th key={headerGroup.id} className="w-14 hover:bg-gray-700">
                <button onClick={handleOpenColumnModal} className="px-4 py-2">
