...
export const useTableUtil = (editor: CustomEditor) => {
    const insertTable = (rows: number, columns: number): void => {
        const [tableNode] = Editor.nodes<Table>(editor, {
            match: (n: CustomElement) => !Editor.isEditor(n) && SlateElement.isElement(n) && n.type === "table",
            mode: "highest",
        });
        ...
        Transforms.insertNodes(editor, { type: "paragraph", children: [{ text: "" }] } as Paragraph, {
            mode: "highest",
        });
    };

    const insertCells = (tableNode: Table, path: Path, action: TableAction): void => {
