...
class AppUtils {
  ...
  dateAdd(date1: any, amount: any, units: any) {
  ...
  /**
	...
	**/
  async notifyOfDueDate({ item, daysBefore, message = `Item is due` }: any) {
  ...
  async notifyAssigned({ item, message = "New Task Assigned" }: any) {
    const to = [item.assignee];
    return await this.notify({ to, template: "default", item, message });
  }
  async notify({ to, template, item, message }: any) {
    ...
    users.forEach((user) => {
      ...
      const msg = pug.renderFile(path + ".message.pug", {
        item,
        user,
        email: user.email,
        message,
      });
      const body = pug.renderFile(path + ".body.pug", {
        item,
        user,
        email: user.email,
        message,
  ...
  async getUsers(toList: any) {
    let users: any = [];
    for (let t = 0; t < toList.length; t++) {
      const to = toList[t];
      if (to.user) {
        to.user.forEach((u: any) => {
          users.push(u);
        });
      } else if (to.group) {
    ...
    let unique = users.filter((value: any, index: any, self: any) => {
  ...
  async getUserNamesInGroup(group: any) {
    const list: any = [];
    const users = await this.server.userService.findUsers({
      userGroups: group,
    });
  ...
  async getUsersInfo(users: any) {
    const usersInfo = [];
    for (let i = 0; i < users.length; i++) {
      const userName = users[i];
      const user = await this.server.userService.findUser({
        userName: userName,
      });
      if (user) usersInfo.push({ userName: userName, email: user.email });
    }
    return usersInfo;
  }
}
function sampleCall(appUtils: any, appServices: any, item: any) {
  appUtils.notify({
    to: [
      {
        user: [
