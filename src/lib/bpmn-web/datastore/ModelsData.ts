...
import { IProcessData, IBpmnModelData, IEventData } from "..";
    ...
    definition.nodes.forEach((n) => {
      if (n.type == "bpmn:StartEvent") {
        ...
        if (timer) {
          ...
          if (!event.expression) event.expression = timer.duration;
